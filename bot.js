...
app.post('/api/groups/:chatId/message', requirePanelToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao enviar mensagem para o grupo.' });
  }
});
...
app.post('/api/users/:userId/message', requirePanelToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao enviar mensagem para o utilizador.' });
  }
});
...
async function registerGroupFromChatMember(upd) {
  try {
    ...
    // 'member', 'administrator' ou 'creator' = bot continua no grupo.
    // 'left' ou 'kicked' = bot saiu/foi removido do grupo.
    const isActiveMember = ['member', 'administrator', 'creator'].includes(newStatus);
...
async function broadcastToGroupsOnly(text) {
  ...
  for (const doc of groupsSnapshot.docs) {
    ...
    try {
      ...
      if (error.response && error.response.statusCode === 403) {
        await db.collection('groups').doc(String(groupId)).update({ active: false, status: 'removed' }).catch(() => {});
      }
    }
  }
    ...
    .onSnapshot(
      (snap) => {
        snap.docChanges().forEach((change) => {
          ...
          processDashboardCommand(change.doc.ref, change.doc.data());
        });
      },
