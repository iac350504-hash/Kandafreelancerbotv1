...
// O painel deve chamar isto repetidamente (ex: a cada 4s) enquanto o admin digita,
// porque o indicador do Telegram expira sozinho ao fim de poucos segundos.
...
app.post('/api/reply', requirePanelToken, async (req, res) => {
  try {
    ...
    await sendWithTyping(userId, `💬 *Resposta do Admin:*\n\n${text}`, { parse_mode: 'Markdown' });
    if (docId) {
...
app.get('/api/stats', requirePanelToken, async (req, res) => {
  try {
    const [groupsSnap, usersSnap, jobsSnap, bansSnap, messagesSnap] = await Promise.all([
...
// Painel admin: broadcast imediato
